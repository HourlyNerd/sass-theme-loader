...
var NodeType = {
  ...
  AT_RULE:        GonzalesNodeType.AtruleType,
...
function ThematicParser(ast, opts) {
  ...
  this.setThemeFunctions(this.themeFunctions);
...
ThematicParser.prototype = {
  ...
  */
  ...
  /**
  * Parses raw Sass variable definitions
  * into the thematic function mapping
  */
  setThemeFunctions: function(themeFunctions) {
    if (themeFunctions) {
      // JSON variables
      themeFunctions = JSON.parse(themeFunctions);

      for (var i in themeFunctions) {
        this.functions[themeFunctions[i]] = true;
      }
    }
  },
  ...
  */
  ...
  _parseNode: function(node, parent) {
    ...
    // Track derivitive locals created by control-flow loops:
      ...
      var str = Node.toString(node);
      var found = false;
      Object.keys(this.functions).forEach(function(fnName){
        if(str.match(RegExp(fnName+"\\(['\"][^'\"]+['\"]\\)"))){
          found = true;
        }
      });
      return found;
    ...
    // Keep "@include mixin-name;" statements for keepable mixins:
      ...
      var target = Node.first(node, NodeType.IDENTIFIER);
    ...
    else if(node.type === NodeType.AT_RULE){
      var foo = Node.toString(node).split(" ")[1];
      if(!foo) return false;
      var importStr = foo.slice(1, -1).split("/").pop();
      // colors is our special thing that im hard-coding in here.
      // anything that starts with _ might contain vars that remain in the the trimmed cod
      // so lets leave those imports
      if(importStr.match(/^(colors|_)[^.]*?(\.scss)?$/)){
        return true;
      }
    }
  ...
  */
  _validateFieldContext: function(parent) {
    ...
    for (var i=0; i < parent.content.length; i++) {
      if (parent.content[i].type === NodeType.OPERATOR) {
  ...
  */
  varsToFieldIdentifiers: function(sass) {
    return sass.replace(/\$([\w-]+)([^:;]*;)/g, function(match, field, terminus) {
      return this.fieldOpen + field + this.fieldClose + terminus;
    }.bind(this));
  },
